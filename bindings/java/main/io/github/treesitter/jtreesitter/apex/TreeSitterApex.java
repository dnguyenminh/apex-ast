package io.github.treesitter.jtreesitter.apex;

import java.lang.foreign.*;

public final class TreeSitterApex {
    ...
    private static final TreeSitterApex INSTANCE = new TreeSitterApex();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterApex() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_apex");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-apex");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-apex");
